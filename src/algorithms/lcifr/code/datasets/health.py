...
class HealthDataset(AbstractDataset):
    ...
    def __init__(self, split, args, normalize=True):
        ...
        if not (True and path.exists(health_file)):
            data_file = path.join(self.data_dir, 'HHP_release3')
            ...
            df_claims = self.preprocess_claims(pd.read_csv(os_join(data_file, 'Claims.csv'), sep=','))
            df_drugs = self.preprocess_drugs(pd.read_csv( os_join(data_file, 'DrugCount.csv') , sep=','))
            df_labs = self.preprocess_labs(pd.read_csv(os_join(data_file, 'LabCount.csv') , sep=','))
            df_members = self.preprocess_members(pd.read_csv(os_join(data_file, 'Members.csv') , sep=','))
        ...
        for key in self.one_hot_columns.keys():
            ...
            unique_val = df_health[key].unique()
