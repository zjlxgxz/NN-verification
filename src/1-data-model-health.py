...
def main(random_seed, is_race_permute, is_sex_permute, is_sex_race_both_permute, 
    ...
    if is_race_permute:
        print ("no race in Health dataset")
        exit()
    ...
    if is_sex_permute:
        ...
        # reference_col_ind = [] 
        # permute_col_ind = [0] # permute 3 features wrt 4-8
        # stratify_permute_row_inplace(X_train, reference_col_ind, permute_col_ind, RS)
        RS.shuffle(X_train[:,[0]])
    ...
    model_save_dir = os_join(res_path, f'health-model_config-{model_config}-max_epoch={max_epoch}-train_bs={train_bs}-random_seed={random_seed}-is_random_weight-{is_random_weight}-race_permute={is_race_permute}-sex_permute={is_sex_permute}-both_sex_race_permute={is_sex_race_both_permute}')
...
def main_based_on_sym_diff(random_seed, is_race_permute, is_sex_permute, is_sex_race_both_permute, 
    ...
    lr = 0.01
    K = 20
    flip_index = 0 #index for male/female #TODO change to race
    print ("flip_feature_index=",flip_index)
    # iteration
    ...
    for _ in range(K):
        ...
        # save model, with dev/test results
        model_save_dir = os_join(res_path, f'health-model_config-{model_config}-max_epoch={max_epoch}-train_bs={train_bs}-random_seed={random_seed}-is_random_weight-{is_random_weight}-race_permute={is_race_permute}-sex_permute={is_sex_permute}'+
